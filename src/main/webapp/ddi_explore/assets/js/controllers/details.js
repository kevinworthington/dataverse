	...
	var populateVariables = function() {
		...
		if ($scope.details.datadscr ){
			...
			for (var i = 0; i < $scope.details.datadscr['var'].length; i++){
				...
				//if we sitll don't havea match see if the name contains a special character which might be translating to something else
				if(!$scope.details.datadscr['var'][i].variable_data ){
					//console.log($scope.details.datadscr['var'][i].name,$scope.details.datadscr['var'][i].name.split("@").join("."))
					$scope.details.datadscr['var'][i].variable_data=$scope._variableData[$scope.details.datadscr['var'][i].name.split("@").join(".")]
...
var traverse = function(o,func) {
	...
	//allow full dataset ddi browsing
	if(detailsURL.uri.indexOf("export")>-1){
		detailsURL.uri=detailsURL.uri+"&exporter=ddi&persistentId="+getParameterByName("persistentId");
		$scope.full_ddi=true;
	}
	...
	//console.log(base_url+file_id+"?format=prep&key="+detailsURL.key)
	var loadcount=0
	if(!$scope.full_ddi){
		$http({
		...
		//we only need to load the one file
		loadcount=1
	}
